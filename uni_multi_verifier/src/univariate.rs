...
pub fn univariate_evaluation_verifier(
    evaluation_proof: EvaluationProof,
    setup: &Setup,
) -> Option<Scalar> {
    let intermediate_layer_commitments = evaluation_proof.dory_proof.intermediate_layer_commitments;
    ...
    let final_values = evaluation_proof.dory_proof.final_values;
    ...
    for round in 0..log2_degree {
        ...
        if round < log2_degree - 1 {
            ...
            let y_logn = (0..log2_degree).fold(Scalar::ONE, |acc, idx| {
                acc * (alpha_vec[idx]
                    + alpha_inv_vec[idx]
                        * r.power_by(&[1_u64 << (log2_degree - (idx + 1)), 0, 0, 0]))
            });
            let h_logn = g1.mul(y_logn);
            let expected_c4 = BlsCurve::tate_pairing(h_logn.to_affine(), final_values.1);
            if c1 == expected_c1 && c2 == expected_c2 && c3 == expected_c3 && v == expected_c4 {
                flag = true
            }
        }
    }
